...
#define EEPROM_NUMBER          100 /* Här sparas talet på dispalyen*/
#define EEPROM_OUTPUT_ENABLED  101 /* Här sparas om displayen är på med (1 = på) eller (0 = av) */
#define EEPROM_COUNT_ENABLED   102 /* Här sparas om talet räknas (1 = på) eller (0 = av) */
#define EEPROM_COUNT_DIRECTION 103 /* Här sparas uppräkings riktningen (1 = upp) och (0 = ner) */
#define EEPROM_INIT			   104 /* Här sparas om vi sparat värden i EEPROM (0 = sant, 0xFF = falskt)*/
...
/********************************************************************************
* Statiska variabler (temporärt ignorerade vid kompilering):
*
...
{
	DDRD = 0xFF; //0b11111111 Sätter alla segment + katoden för displayen till utportar
	DDRC |= (1 << DISPLAY2_CATHODE);
	DISPLAY1_OFF;
	DISPLAY2_OFF;
	
	timer_init(&timer_digit, TIMER_SEL_1, 1);
	timer_init(&timer_count_speed, TIMER_SEL_2, 100);
	check_eeprom_values(); //Vi kollar om det finns gamla värden och läser i så fall av dom
...
*	
*				1. Räknar upp löpt tid via timer timer_count_speed
*				2. När timer löper ut sker upp/nedräkning
*				3. Vid uppräkning inkrementera variabeln number upp till
*				   och med aktuell maxvärde max_count, annars nollställer
*
********************************************************************************/
...
/************************************************************************
* check_eeprom_values: Kollar  om det finns några värden i EEPROM
*					   Om de finns läses dessa in och displayen försätts
*					   i samma tillstånd som innan programmet avslutatdes.
*
*					   1. Vi kollar om EEPROM är initierar. Då ligger värdert 0
*						  på EEPROM_INIT, annars ligger startvärdet 0XFF
*					   2. Om EEPROM är initierat läses värdet in, annars
*						  sätts adresserna till starttillstånd
*					   3. Först läser det senaste tal in och sparas i variabeln
*						  number. Tiotalet och entalet sparas i variabeln digit1
*						  samt digit2 via anrop ac display_set_number    
*					   4. Läser in uppräkningsriktiningen och sparar i variabeln
*						  count_direction. Det inlästa talet typomvandlas till
*						  enum display_count_direction (god kutym).                   
*					   5. Om displayen var påslagen innan programmet
*						  avslutades tänds de       
*					   6. Om uppräkningen var på slagen innan programmet
*						  avslutades                                  
************************************************************************/
static inline void check_eeprom_values(void)
{
